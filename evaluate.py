...
def _bicep_curl(pose_seq):
    ...
    print('\n\nExercise arm detected as: {}.'.format(side))
    ...
    if upper_arm_torso_range > 35.0:
        correct = False
        feedback += ' \nExercise could be improved: Your upper arm shows significant rotation around the shoulder when curling. Try holding your upper arm still, parallel to your chest, ' + \
                    'and concentrate on rotating around your elbow only.\n\n'
    
    if upper_arm_forearm_min > 70.0:
        correct = False
        feedback += ' Exercise could be improved: You are not curling the weight all the way to the top, up to your shoulders. Try to curl your arm completely so that your forearm is parallel with your torso. It may help to use lighter weight.\n\n'
